...
<div class="row">
    ...
    <form method="post" action="{{ route('posts.store') }}" enctype="multipart/form-data" id="form">
        <!-- @csrf => to make csrf token -->
        <input type="hidden" name="_token" value=" {{ csrf_token() }}"/> <!-- same as upper for csrf token -->
    <div class="mb-3">
        <label class="form-label">Title</label>
        <input type="text" class="form-control" name="title" placeHolder="Enter The Title" value="{{ old('title') }}">
    </div>
    <div class="mb-3">
        <label class="form-label">Description</label>
        <textarea type="text" class="form-control" name="description" placeHolder="Enter The Description">{{ old('description') }}</textarea>
    </div>
    <div class="mb-3">
    <label class="form-label">Published</label>
        <select name="is_publish" class="form-control">
            ...
            <option @selected(old('is_publish') == 2) value="2">No</option>
        </select>
    </div>
    <div class="mb-3">
